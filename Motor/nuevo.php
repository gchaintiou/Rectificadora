...
if(!$guardar)
	MOTMostrarEdicion($conex, 0);
else
	{
	// Levanto los datos del encabezado
	// Busco el numero de motor para asignarlo
	$result = $conex->Execute("SELECT nro_motor FROM mote ORDER BY nro_motor DESC LIMIT 1");
	$nro_motor = $result->fields['nro_motor']+1;
	$desc_motor = $_POST['desc_motor'];
	$lista = $_POST['lista'];
	
	// Guardo los datos del encabezado
	$query = "INSERT INTO mote(nro_motor, desc_motor, nro_lista) VALUES($nro_motor, '$desc_motor', $lista)";
	$conex->Execute($query);
	
	// Guardo los items seleccionados para el motor
	$result = $conex->Execute("SELECT * FROM mobe ORDER BY numero, seccion ASC");
	while(!$result->EOF)
		{
		$numero = $result->fields['numero'];
		if(isset($_POST['item'.$numero]))
			if($_POST['item'.$numero])
				$conex->Execute("INSERT INTO motd(nro_motor, numero) VALUES($nro_motor, $numero)");
		$result->MoveNext();
		}
	
	// Informo que se guardo
	$result = $conex->Execute("SELECT mobe.descripcion FROM mobe INNER JOIN motd WHERE motd.nro_motor=$nro_motor AND mobe.numero=motd.numero");
	?>
	<div align="center" class="titulo1">El Siguiente Motor ha Sido Guardado con Exito:</div>
	<div>&nbsp;</div>
	<table width="40%">
	 <tr>
		  <td width="44%" class="head_item_std1">Numero del Motor:</td>
		  <td width="56%" class="head_item_std2"><?php echo $nro_motor?></td>
	 </tr>
	 <tr>
		  <td class="head_item_std1">Descripcion del Motor:</td>
		  <td class="head_item_std2"><?php echo $desc_motor?></td>
	 </tr>
	 <tr>
		  <td class="head_item_std1">Lista de Precios: </td>
		  <td class="head_item_std2"><?php echo $lista?></td>
	 </tr>
	</table>
   <div>&nbsp;</div>
   <div align="left" class="titulo1">Items:</div>
	<?php
	while(!$result->EOF)
		{
		MostrarLinea($result->fields['descripcion'], "etiqueta1");
		$result->MoveNext();
		}
	?>
	...
	<tr>
		  <td width="48%" align="right"><input name="cancel" type="button" value="   Inicio   " onClick="MOTCancelar()" /></td>
		  <td width="5%">&nbsp;</td>
		<td width="47%" align="left"><input name="aceptar" type="button" value="    Nuevo    " onClick="MOTNew()" /></td>
	</tr>
	</table>            
	<?php
	}
