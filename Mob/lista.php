...
switch($opc)
	{
	case 0:		// Cambiar una lista completa por otra multiplicada por un factor 
		{
		MOB_PantallaEdicionGlobal($user_gr->GetConex(), $opc);
		break;
		}
	
	case 1:		// Modificar los precios de items particulares de una lista
		{
		$conex = $user_gr->GetConex();
		isset($_GET['lista']) ? $lista = $_GET['lista'] : $lista=0;
		
		MOB_PantallaEdicionItem($conex, $opc, $lista);
		if($lista)	MOB_MostrarListaEdicion($conex, $opc, $lista);
		break;
		}
	}
?>
